...
class Args():
    def __init__(self):
        ...
        self.loss_function = "BCELossWithNegativeSample" # "CustomContrastiveLoss" "MeanSquareError" "CosSimilarity" "LossWithNegativeSample" "BCELossWithNegativeSample"
        self.batch_size = 64
        self.lr = 0.00005
        self.maxlen = 50
...
model = SASRecF(args).to(args.device) # no ReLU activation in original SASRec implementation?
...
for epoch in range(epoch_start_idx, args.num_epochs + 1):
    ...
    for step, (padded_embeddings, padded_pos_emb, padded_neg_emb, pad_mask, ad_ids, padded_ad_tokenidx_seqs) in enumerate(tqdm(dataloader)):
        logits = model(padded_embeddings, pad_mask, padded_ad_tokenidx_seqs)
