...
class TrainDataset(Dataset):
    def __init__(self, args):
        self.args = args
        # 添加负采样数量参数，默认为5
        self.num_neg_samples = getattr(args, 'num_neg_samples', 5)
    ...
    def collate_fn(self,batch):
        ...
        pad_mask = []
        padded_pos_embs = []
        padded_neg_embs = []  # 将变成 [batch_size, seq_len, num_neg_samples, emb_dim]
        padded_ad_ids = []
        ...
        for idx,emb in enumerate(ad_embeddings):
            ...
            # 生成多个负样本
            random_neg_ids = self.generate_multiple_random_numbers(
                0, self.lenth_unit_data-1, [ad_ids[idx][1:]], emb_len, self.num_neg_samples)
            
            # 构建负样本embedding: [seq_len, num_neg_samples, emb_dim]
            random_neg_emb_list = []
            for pos_idx in range(emb_len):
                neg_embs_for_this_pos = []
                for neg_idx in range(self.num_neg_samples):
                    neg_id = random_neg_ids[pos_idx][neg_idx]
                    neg_embs_for_this_pos.append(self.unitid_data[unit_data_map_ids[neg_id]]['embedding'])
                random_neg_emb_list.append(neg_embs_for_this_pos)
            
            random_neg_emb = paddle.to_tensor(random_neg_emb_list, dtype='float32')  # [seq_len, num_neg_samples, emb_dim]
            
            # 处理padding
            if padding_len:
                # 为padding部分创建零向量: [padding_len, num_neg_samples, emb_dim]
                padding_neg_vector = paddle.zeros([padding_len, self.num_neg_samples, self.args.emb_dim], dtype='float32')
                padded_neg_emb = paddle.concat([padding_neg_vector, random_neg_emb], axis=0)
            else:
        ...
        padded_neg_embeddings = paddle.stack(padded_neg_embs,axis=0)  # [batch_size, seq_len, num_neg_samples, emb_dim]
    ...
    def generate_multiple_random_numbers(self, start, end, exceptions_list, seq_len, num_neg_samples):
        """
        为序列中的每个位置生成多个负样本
        ...
        Args:
            ...
            exceptions_list: 不能生成的数值列表（正样本序列）
            seq_len: 序列长度
            num_neg_samples: 每个位置的负样本数量
            
        Returns:
            List[List[int]]: [seq_len, num_neg_samples] 的二维列表
        """
        all_exceptions = set()
        for exc_list in exceptions_list:
        ...
        for pos_idx in range(seq_len):
            ...
            neg_samples = []
            attempts = 0
            max_attempts = num_neg_samples * 10  # 防止无限循环
            
            while len(neg_samples) < num_neg_samples and attempts < max_attempts:
                num = random.randint(start, end)
                if num not in all_exceptions and num not in neg_samples:
                    neg_samples.append(num)
                attempts += 1
            
            # 如果还没有足够的负样本，随机填充（避免死循环）
            while len(neg_samples) < num_neg_samples:
                num = random.randint(start, end)
                if num not in neg_samples:
                    neg_samples.append(num)
...
class TestDataset(Dataset):
    def __init__(self, args):
        ...
        self.gt_data = []
        self.test_data = []
        self.unitid_data,self.lenth_unit_data = self.load_unit() if args.dataset_dir == "data" else self.load_unit_w()
